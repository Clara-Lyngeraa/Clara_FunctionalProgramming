module StateMonad
    ...
    let characterValue (pos : int) : SM<char> =
        S (fun s ->
            match pos with
            | p when p >= s.word.Length || p < 0 ->  (Failure (IndexOutOfBounds pos))
    ...
    let declare (var : string) : SM<unit> =
       S (fun s ->
           ...
           else if List.item 0 s.vars |> Map.containsKey var then Failure (VarExists var)
           else Success ((),  {s with vars = (Map.add var 0 s.vars.Head) :: s.vars})
       ) 
        
            
    let update (var : string) (value : int) : SM<unit> =
         let rec aux listBefore listNow =
            match listNow with
            | []      -> None
            | m :: ms -> 
                match Map.tryFind var m with
                | Some v -> Some (listBefore @ Map.add var value m :: ms)
